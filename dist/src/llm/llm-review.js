...
import { ComponentReviewSchema } from './structured/review.js';
 ...
 */
function translateLLMError(err) {
 ...
 */
function buildUserContent(componentTree) {
...
async function callAnthropic(config, systemPrompt, userContent) {
    ...
    if (!textBlock) {
        throw new Error(`No text block in Anthropic response. Content types: ${result.map((b) => b.type).join(', ')}`);
    }
 ...
 */
export async function runLLMReview(componentTree, config) {
