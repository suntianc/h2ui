...
export async function convertCommand(file, options, configFile = {}) {
    if (!file) {
    ...
    if (options.llm || configFile.llm) {
        llmConfig = {
            provider: (options.llmProvider ?? configFile.llm?.provider ?? DEFAULT_LLM_CONFIG.provider),
            model: options.llmModel ?? configFile.llm?.model ?? DEFAULT_LLM_CONFIG.model,
            mode: (options.llmMode ?? (options.llm ? 'always' : configFile.llm?.mode ?? DEFAULT_LLM_CONFIG.mode)),
