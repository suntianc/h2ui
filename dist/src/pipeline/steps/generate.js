...
async function formatCode(code, isTypescript) {
    try {
        ...
        console.warn(`Prettier formatting failed: ${err.message}. Using unformatted code.`);
        return code;
    }
}
/**
 ...
 */
function generateCSSModule(name, properties) {
 ...
 */
async function writeFormattedFile(outputPath, content, formatAs) {
            ...
            try {
                ...
                if (ctx.cssFiles) {
                    for (const cssFile of ctx.cssFiles) {
                        if (!cssFile.css || !cssFile.css.trim())
                            continue;
