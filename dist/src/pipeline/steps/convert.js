 ...
 */
function generateJsxFromNode($, el, warnings) {
    if (el.type === 'text') {
        const text = el.data;
        return text || '';
    }
 ...
 */
function generateComponent($, componentName, isTypescript, warnings) {
    const bodyEl = $('body');
    let innerContent;
    if (bodyEl.length > 0) {
 ...
 */
function generateComponentCode($, node, isTypescript, allComponents, warnings) {
    const lines = [];
 ...
 */
function generateRootComponent($, root, allComponents, isTypescript, warnings) {
    const lines = [];
    // Import child components
 ...
 */
function extractCssProperties($, el) {
    const result = {};
        ...
        try {
            ...
            if (ctx.componentTree) {
                ...
                for (const node of flatNodes) {
                    ...
                    const code = generateComponentCode($, node, ctx.options.typescript, components, newCtx.warnings);
                ...
                const rootCode = generateRootComponent($, ctx.componentTree, components, ctx.options.typescript, newCtx.warnings);
                components.unshift({
